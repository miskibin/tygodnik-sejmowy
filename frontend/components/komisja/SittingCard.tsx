  ...
  if (variant === "compact") {
    ...
    return (
      <li className="border-b border-border last:border-b-0 py-3">
        ...
        {oneLine && (
          <p className="text-[13px] leading-snug text-muted-foreground mt-1 max-w-[820px] line-clamp-2">
            {oneLine}
          </p>
        )}
      </li>
  ...
  return (
    <article className="border border-border bg-background p-4 md:p-5">
      <header className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-3">
        <h3 className="text-[18px] md:text-[20px] font-medium leading-none tracking-[-0.01em] m-0">
          {dateLabel}
        </h3>
        ...
        {sitting.closed && (
          <span className="font-sans text-[11px] text-muted-foreground border border-border px-1.5 py-0.5 font-medium">
            zamknięte
          </span>
        )}
        {sitting.remote && (
        ...
        {sitting.videoPlayerLink && status !== "PLANNED" && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="ml-auto font-sans text-[12px] tracking-[0.04em] text-foreground underline decoration-dotted underline-offset-4 hover:no-underline"
          >
            ▶ Nagranie wideo
          </a>
        )}
      ...
      {agenda.length > 0 ? (
        ...
        <p className="text-[14.5px] leading-relaxed text-secondary-foreground m-0 max-w-[820px]">
          {sitting.agendaText}
        </p>
      ) : (
