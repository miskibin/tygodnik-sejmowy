...
import { getMpStatementsRows } from "@/lib/db/posel-tabs";
...
export async function GET(req: Request, ctx: { params: Promise<{ mpId: string }> }) {
  ...
  try {
    const rows = await getMpStatementsRows(mpId, TERM, offset, limit);
    return NextResponse.json(rows);
  } catch (e) {
    console.error("[api/posel/statements]", { mpId, offset, limit, e });
