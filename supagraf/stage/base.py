"""Fixture-folder → stage-table upserter for the external sources
(districts, postcodes, promises, mp_office_expenses).

Reads every JSON file under `fixtures/<subdir>/<resource>/`, validates with the
caller-supplied Pydantic model, and bulk-upserts into `_stage_<resource>` keyed
on `(term, natural_id)`. Sejm/ELI resources no longer go through files —
see supagraf/sync/.
"""
